...
void Qodot::load_map(const String &map_file_str) {
	...
	map_parser.map_parser_load(map_file.get_data());
}
...
void Qodot::set_entity_definitions(Dictionary p_entity_defs) {
	Array keys = p_entity_defs.keys();
	Array vals = p_entity_defs.values();
	for (int i = 0; i < keys.size(); i++) {
		Dictionary valDict = Dictionary(vals[i]);
		String key = String(keys[i]);
		int value = valDict.get("spawn_type", static_cast<int>(EST_ENTITY));
		CharString key_cs = key.utf8();
		map_parser.map_data->map_data_set_spawn_type_by_classname(key_cs.get_data(), value);
	}
}

void Qodot::set_worldspawn_layers(Array p_worldspawn_layers) {
	for (int i = 0; i < p_worldspawn_layers.size(); i++) {
		Dictionary worldspawn_layer = Dictionary(p_worldspawn_layers[i]);

		bool build_visuals = false;
		String str = "NONE";
		CharString texture = str.utf8();
		
		Array keys = worldspawn_layer.keys();
		Array vals = worldspawn_layer.values();

		for (int k = 0; k < keys.size(); k++) {
			String key = keys[k];
			if (key == "texture") {
				String value = String(vals[k]);
				texture = value.utf8();
			} else if (key == "build_visuals") {
				build_visuals = bool(vals[k]);
			}
		}
		map_data->map_data_register_worldspawn_layer(texture.get_data(), build_visuals);
	}
}

void Qodot::generate_geometry(Dictionary p_texture_dict) {
	Array texture_keys = p_texture_dict.keys();
	Array texture_vals = p_texture_dict.values();
	for (int i = 0; i < texture_keys.size(); i++) {
		String texture_key = String(texture_keys[i]);
		Vector2 value = Vector2(texture_vals[i]);
		...
		map_data->map_data_set_texture_size(texture_key_cs.get_data(), width, height);
	}
...
void Qodot::gather_texture_surfaces_internal(const String p_texture_name, const String p_brush_filter_texture, const String p_face_filter_texture, bool p_filter_layers) {
	...
	surface_gatherer.surface_gatherer_set_texture_filter(texture_name.get_data());
	surface_gatherer.surface_gatherer_set_brush_filter_texture(brush_filter_texture.get_data());
	surface_gatherer.surface_gatherer_set_face_filter_texture(face_filter_texture.get_data());
